...
5. Verify seam continuity (joins are ordinary day-over-day moves), correct listing dates, and `len(px[t]) == len(dates)` for all 91.
...
- **Foreign (19):** `LSEG LN→LSE:LSEG`, `DB1 GY→XTRA:DB1`, `ENX FP→ENXTPA:ENX`, `EXPN LN→LSE:EXPN`, `WISE LN→LSE:WISE`, `ADYEN NA→ENXTAM:ADYEN`, `PGHN SW→SWX:PGHN`, `EQT SS→OM:EQT`, `CVC NA→ENXTAM:CVC`, `DWS GY→XTRA:DWS`, `AMUN FP→ENXTPA:AMUN`, `ICG LN→LSE:ICG` (**not** `ICP` — mis-resolves to a Kazakhstan listing), `FTK GY→XTRA:FTK`, `BGN IM→BIT:BGN`, `FBK IM→BIT:FBK`, `SAVE SS→OM:SAVE` (Nordnet), `IGG LN→LSE:IGG` (IG Group), `AJB LN→LSE:AJB` (AJ Bell), `AZA SS→OM:AZA` (Avanza).
...
## 8. Coverage universe — 7 sub-sectors, 91 tickers
...
Wealth & Brokers (19): SCHW US · LPLA US · HOOD US · IBKR US · COIN US · RJF US · SF US · BGN IM · FBK IM · FTK GY · CRCL US · FIGR US · ETOR US · WLTH US · SQN SW · AZA SS · SAVE SS · IGG LN · AJB LN
```
Excluded from the P/E panel: none. (The vendor re-tickered `RELY LN`→`RELY US` (Remitly) and `SQ SW`→`SQN SW` (Swissquote); both were added to the universe on 2026-06-25, taking it from 85 to 87. On **2026-07-13** four European online brokers — **Avanza `AZA SS`, Nordnet `SAVE SS`, IG Group `IGG LN`, AJ Bell `AJB LN`** — were added to Wealth & Brokers, taking it to 91. (Avanza came in a same-day second upload: the first workbook's `AVZ SS` column was empty, so it was initially dropped per the sourced-data-only rule; the re-upload populated Avanza under **`AZA SS`**, which was then added.)
...
7. **Software is reference-only and isolated** — `SW` must never be merged into `DATA`/the 91-name universe or the CLUSTERS; the Software tab must not feed pairs/screens/backtest.
...
- **US market holidays** look like ordinary weekday rows in the Bloomberg workbook (forward-filled) — they are **not** settled closes. Juneteenth, Labor Day, Thanksgiving, July 4th, etc. must be excluded from `ASOF` just like today's intraday row. **Diagnostic:** count how many names change vs the prior row. A settled US close moves ~all 91; a **US holiday moves only the 20 European names** (plus the odd US name on a vendor estimate revision); an early intraday pull moves a handful. (Seen 2026-09-07, Labor Day: 20 foreign + 4 US changed, so `ASOF` stayed Fri 4 Sep. **The same signature can appear on an ordinary trading day** when the pull's US pricing simply has not refreshed: on the 2026-09-09 workbook, Tue 8 Sep was a real US session yet 64 of the 71 US names were byte-identical to 4 Sep while all 20 Europeans had moved. Treat it identically — it is not a settled close. For a *relative-value* panel the mixed case is the worse one: advancing `ASOF` there would pair a current European leg against a stale US leg in every cross-border pair.)
...
- **`MQ US` sits on an F = 0.25 basis** — the reference basis holds Marqeta at a quarter of the S&P pull's price (e.g. 4.14 vs 16.58 on 2026-09-04), a 4:1 factor inherited from an early pull. F is stable (max dev 0.12%), so the splice is correct, but 2-dp rounding on a ~$4 price costs ~0.25% per observation, which is why MQ is the one name whose spliced day-over-day move can differ from the pull's by ~0.2pp. Harmless for `DPX`'s only consumer (click-to-measure reports *returns*, which are scale-invariant) — but if precision there ever matters, rebase MQ to F = 1.0 and re-round; that touches only MQ and leaves every reported return unchanged apart from less rounding noise.
- **A US market holiday inside `NEW_DATES`** breaks a naive splice guard. The splice template asserted all 71 US names were priced on every new date, which is right for an ordinary day but fires on a good pull when the US market was shut (S&P returns no close at all). `splice_910.py` discriminates: **no** US name priced = holiday, forward-fill (and assert some non-US name did trade, else the pull really is bad); **some** priced and others not = incomplete pull, halt. Carry that guard into future splice scripts.
- **Token substring order** — `__DATA__` ⊂ `__QDATA__`/`__BTDATA__`; replace the long ones first (§3).
- **`bt_verify` is self-consistent** — regenerate `bt_results.json` (`bt_export.py`) whenever backtest inputs change, or the gate "fails" only because the reference is stale. **Backtest headlines can move on an ordinary daily refresh** even though `btdata.json` is untouched: the backtest resamples P/E to month-end, so a sourced revision that lands on a *completed* month-end value shifts that month's signal. Trace it by diffing month-end P/E between the old and new `data.json` before assuming anything broke (2026-09-25: FIS/CG 2024 and WISE/FOUR 2026 month-end revisions moved the 6m buy-the-discount mean +5.44% → +5.49%, n 864 → 863; Sharpe unchanged at 1.21).
...
- **Single-day vendor blow-ups — the `DROP` block.** Separate from the `SQN SW` HOLD above. A workbook can print a single observation that is simply wrong, with no earlier workbook holding a good value for that same date (because the date is brand new). HOLD cannot help there — there is nothing sourced to re-pin to — and carrying the prior day forward would be fabrication, so §7's hard rule ("if a series is unavailable, drop the name") makes **dropping the observation to `null`** the correct treatment. `mkdata.py` carries a `DROP = {ticker: [dates]}` block when one is needed; **it is currently empty/absent, which is the normal state.** **Seen 2026-09-11: `TOST US` printed 0.84x against 24.16x the day before (−96.5% overnight) while Toast's share price moved +0.6% (31.94 → 32.12)** — an FY+1 EPS wrong by ~29x. **The vendor corrected it three days later** (the 2026-09-15 workbook restated 11 Sep to 24.29x, between 24.16x on 10 Sep and 25.18x on 14 Sep), and the block was removed on 2026-09-15 — so a `DROP` entry is a temporary hold, and **each refresh should re-check whether its dates have been fixed and remove the entry once they have.** **The discriminator is always price:** a real one-day multiple move is reproduced by the share price almost exactly (that same day `FTK GY` −7.9% P/E vs −7.88% price, `FISV US` +5.4% vs +5.42%), and the annual FY+1 roll (§9b) steps by a growth rate of order 10%, never 96%. A trailing `null` is safe — `lastVal()` scans back to the last non-null, the Grid pre-filters, and `backtest.py`'s month-end resample skips nulls — so the name falls back to its previous observation instead of showing a broken one.
- **Always outlier-scan the new day before publishing**, not just the historical diff: flag any name moving more than ±8% day-over-day or landing below 3x / above 200x, then cross-check each flag against the S&P price move. The historical-revision diff would never have caught the `TOST US` case, because the bad value was on the *new* date.
- **A fresh container has no `*.xlsx`** — the workbooks are gitignored (licensed vendor data, never committed), so `mksw.py` dies with `FileNotFoundError: bbg_software_mults.xlsx` whenever the session restarts and only the coverage workbook was re-uploaded. `realign_sw.py` is the fallback: `mksw.py`'s alignment step is a pure date-keyed lookup, so it can be reproduced exactly from the PREVIOUS `sw_data.json` plus the previous date axis (`/tmp/data_prev.json`) — re-key the stored values by their own dates, re-emit against the new axis. That re-indexes already-sourced values and invents nothing (§7 hard rule); the newly added trading day is after `SW_ASOF` anyway, so it comes out `null` exactly as `mksw.py` emits it. Run `mksw.py` whenever the workbook is present; this is only the fallback. **Also reinstall deps on a fresh container** (`pip install openpyxl pandas`, and `npm install playwright` in the scratchpad with `node_modules` symlinked into the repo for `mkcharts.mjs`).
- **When the S&P connector is unavailable, do NOT forward-fill the missing trading day.** The splice scripts' realign loop forward-fills, which is right for an exchange *holiday* (the market was shut, there is no close, carrying the prior one forward is the convention) but wrong for a real session whose closes simply could not be fetched — that presents a stale price as the day's close and silently corrupts every click-to-measure return ending on it. Use `realign_dpx_nofill.py`: re-key the stored prices by their own dates, re-emit against the new axis with **no fill past the last sourced date**, and leave `daily_px.json`'s `asof` at that last sourced date. `template.html` already handles the lag — `PXASOF = DPXMETA.asof` and `pxLagNote()` render a red banner saying prices run a day behind and the latest date was left blank rather than carried forward. **The P/E panel still advances**, because it comes from the uploaded workbook and needs no connector; only `DPX` lags, and `DPX` feeds nothing but click-to-measure. Backfill with the normal splice once the connector is authorized. (Seen 2026-09-20: connector disconnected and requiring re-auth, which a non-interactive session cannot perform.)
- **A vendor restatement can be TRANSIENT and revert exactly — price-independence cannot tell you which.** `KKR US` sat at 14.08x for 2026-09-09 in the 16 Sep workbook; the 17 Sep workbook cut the 09-09..09-16 block ≈ −2.7%, it held there through the 18 Sep workbook, and the **21 Sep workbook restored every date to the cent** (14.08x again, 0.00% across all six). At the time the cut showed the textbook restatement signature — contiguous block, tight uniform %, share price moving independently (−7.0% across the block) — and that signature was doing its job: it distinguishes a restatement from a price-driven artifact. What it *cannot* distinguish is a durable re-estimation from a transient one that will revert; only later workbooks settle that. **No special handling is needed** — taking each workbook as truth (§7a) puts the values back on its own, which is exactly what happened here, and the panel is correct at every vintage. But do not describe a fresh block as a settled restatement: call it what it is on the day, and expect some to reverse. Distinct from the `SQN SW` HOLD (a glitch that persisted) and the `TOST US` DROP (a single-day blow-up on a brand-new date).
- **Backfilling a blanked day is a different splice shape** — the axis does **not** move, so base is the *current* `daily_px.json` (not a previous-axis snapshot) and only the blank date is filled; `splice_920.py` is the worked example. **The substitute checks held up:** when the 2026-09-18 prices were backfilled, the real cross-check reproduced the connector-down verdicts to within 0.02pp on every flagged name, and the two "unchanged" names turned out to have had exactly flat prices. The substitutes below are sound, but confirm them against price once the connector returns.
- **The §10 price cross-check needs the connector.** With it down, an outlier flag on the new day cannot be confirmed against price. Substitutes that work from the panel alone: (a) **how routine is a move that size for that name** — `COIN US`/`HOOD US` exceed ±8% on ~10% of all days with a p99 |move| near 25%, so a +9..12% day is unremarkable for them; (b) **did the whole peer complex move together** — a coherent same-direction gradient across crypto-linked names (COIN +11.7%, HOOD +9.1%, CRCL +7.9%, FIGR +5.3% on 2026-09-18) is a sector move, the opposite of the `TOST US` signature of one isolated name moving while peers stayed flat; (c) for an *unchanged* name, how often it prints identical consecutive values historically (`BILL US`/`WLTH US` ≈ 4.8%, so two of 71 on a day is expected). State plainly that the price check could not be run.
- **Judge a name's "unexplained" daily move against its OWN baseline, not the universe's.** The price cross-check compares the day's P/E move with the day's price move; the gap is the day's change in the FY+1 EPS estimate. On a typical day the universe median gap is ~0.02pp, but names with an actively moving consensus run far wider every day: `WISE LN`'s own median daily gap since 2025 is 0.31pp with a p90 of 1.07pp, so its 0.77pp gap on 2026-09-23 (an EPS estimate uptick of ~0.8%) was exceeded on one day in six and is ordinary drift, not a fault. Compute the name's own distribution before treating a sub-1pp gap as suspect; a genuine blow-up (`TOST US`, §10 DROP) is orders of magnitude larger. Related diagnostic for the change-count check: one unmoved European name per day is normal **if the name rotates** (`BGN IM` 21 Sep, `DB1 GY` 22 Sep, `FBK IM` 23 Sep — FBK's price was also exactly flat); the **same** name unmoved day after day would point to a stale feed.
- **Check panel alignment by DATE, not by length.** When `mksw.py` silently fails to run, `sw_data.json` keeps the *previous* axis — and because the axis is a fixed-width rolling window, it is still 1,565 long, so a length check passes while every value sits one trading day off. The checks that actually catch it: `sw['asof'] == data['asof']`, and a spot-check that a known value lands on its own date (e.g. the `SW_ASOF` row is non-null and everything after it is null). Same applies to `daily_px.json`.
...
- **`asof` = 2026-09-24**; `dates` span **2020-09-25 → 2026-09-24** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`data.json`, `daily_px.json` and `sw_data.json` are all aligned to 2026-09-24.**
- Most recent change (**2026-09-25**) advanced the panel one trading day to Thu 24 Sep: **all 71 US and all 20 European names moved**, none of the US values matching 23 Sep. Fri 25 Sep is the pull-day row (4 of 71 US and 0 of 20 European names changed) and is excluded. The rolling window dropped 2020-09-24, so the row count is unchanged.
- **Sourced revisions: 5 names, largest single-date change 2.62%**, all with the restatement signature (contiguous or tight, price moving independently, no cliffs):
  - **`FOUR US` — a fresh uniform ≈ +2.0% re-base of the whole 08-07..09-23 window (34 contiguous dates, IQR +1.85..+2.04%, price −6.8% across), NOT a KKR-style reversal.** It happens to offset the −1.94% cut the 21 Sep workbook made on 09-08..09-18, so some of those dates land back on their pre-cut values — but dates that were never cut (08-07, 08-28) moved ≈ +2% too, the previously-cut and never-cut dates moved alike (+2.02% vs +1.90%), and only 5 of the 9 previously-cut dates land *exactly* on their old value. The discriminator from `KKR US`: KKR's revert touched only the cut dates and restored every one to the cent.
  - **`WISE LN` — revised in the same direction for a second round running**, now over 49 contiguous dates 07-17..09-23 (IQR +2.07..+2.42%, price −6.3% across). Cumulatively vs the 18 Sep vintage: July ≈ +2.1%, August ≈ +2.35%, September ≈ +4.2%. An actively falling FY+1 EPS consensus; recorded as such, not as settled.
  - `FIS US` 51 contiguous dates 2024-02-29..05-09 at −1.71% (price +6.5% across); `CG US` 30 dates at ≈ +0.8% (28 of them Feb–Mar 2024, plus two stray single dates); `ARES US` 1 date +0.06%.
  All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself; `KKR US` still at its restored values.
- The new-day outlier scan was clean: median |move| 1.40%, **nothing beyond ±8%**, no level below 3x or above 200x. The largest movers reconcile to price within 0.06pp (`AFRM US` +5.12% vs +5.11%, `FIGR US` −5.79% vs −5.77%, `PGHN SW` −5.29% vs −5.35%, `TRU US` −4.80% vs −4.81%), **except `CHYM US`: P/E +6.50% on a +2.31% price move, a 4.19pp gap (a ~3.9% FY+1 EPS cut)**. Judged against Chime's own baseline per §10 that is ordinary — a June-2025 IPO with thin, fast-moving coverage, its daily gap is ≥ 4.19pp on 13.4% of days (p90 7.8pp). It is not a glitch V either: 23 Sep's −7.55% was fully price-explained, and 24 Sep is a partial price bounce plus a genuine estimate cut. `TOST US`'s 1.72pp gap is exceeded on 11.7% of its days.
- The last spliced price day is **24 Sep** (`splice_925.py`); the seam reproduced the pull's own move for 90 of 91 names, `MQ US` differing 0.127pp on the F=0.25 rounding artifact (§10).
- **`sw_data.json` is realigned with `realign_sw.py`, not rebuilt**, on any container without `bbg_software_mults.xlsx` (gitignored, so every fresh container). See the §10 note; the realign is a pure date re-index of already-sourced values, verified by `sw['asof'] == data['asof']` and the `SW_ASOF` row landing on 2026-08-19.
- **Mon 7 Sep (US Labor Day) sits in the panel as an interior row**, US forward-filled and Europe live — the six-year precedent (§10). The `ASOF` cap protects the *published* date; an interior holiday is a known one-day artifact. **Tue 8 Sep is the stale-US-pricing row** described in §10 — a real session where the vendor had not refreshed 64 of 71 US names; it too is interior and forward-filled, and was correctly never published as `ASOF`.
- Backtest headlines on the 74-name tradeable universe: **6-month buy-the-discount reversion ≈ +5.49% (863 pair-months); portfolio Sharpe ≈ 1.21** (n = 48). The gate passes 39/39. The 6m figure moved from +5.44% (864) on 2026-09-25 because this round's sourced revisions changed completed month-end P/E values (FIS/CG 2024, WISE/FOUR 2026) — see the §10 `bt_verify` note.
- The **software panel is stale by design**: last sourced observation **2026-08-19**, capped by `SW_ASOF` in `mksw.py`. It only moves when a fresh `bbg_software_mults.xlsx` is supplied (§7f); the tab self-discloses the lag.
- **A daily email routine is live** — Claude routine `trig_017Ew43keHj4u8yXBzJ7vLfQ`, 08:00 London, fresh session per firing, running `python pair_email.py --send`. It emails premium/discount charts for **18** curated pairs (**`HLI US`/`PJT US` added 2026-09-18**) plus absolute charts for XYZ/ADYEN/CHYM/**MRX** (Marex added 2026-09-16), subject prefixed "Relative P/E Agent". The chart PNGs are rendered from the gate-passed dashboard during the Pages deploy (`mkcharts.mjs` → `_site/charts`), never committed. **The pair/single lists live in THREE places that must move together** — `SINGLES`/`PAIRS` in `pair_email.py`, the same two arrays in `mkcharts.mjs`, and the render-count assertion in `.github/workflows/pages.yml` (currently `-eq 22` = 18 pairs + 4 singles). Miss the workflow count and the deploy fails; miss `mkcharts.mjs` and the email links a 404. A one-shot (`trig_01PL9sLBobUMTHi44f8F5uBY`, 2026-10-25) shifts the cron for the BST→GMT change.
- **Refresh lesson worth keeping** (2026-09-09, three uploads in one day): a workbook can run *past* the last settled close without containing it. Always run the §10 change-count diagnostic before setting `ASOF` — the master `Coverage_dashboards.xlsx` carries only 8 names and cannot drive a refresh at all, and one `Coverage_PE_multiples.xlsx` pull had 64 of 71 US names byte-identical to four days earlier.
- **Open decision:** whether to **freeze the expanding-z baseline at 2021-06** (recovering 6m ≈ +2.98%, Sharpe ≈ 0.71) vs. let it float to 2020 (current). To freeze: constrain `PEMONTHS` to `>= '2021-06'` in `backtest.py`/`bt_export.py` (and optionally `q_pairs.py`), regenerate `bt_results.json`, re-run the gate.
- **Open decision (§9b):** whether to roll-adjust the panel for the discrete annual FY+1 roll so level statistics become continuous. Would change every published percentile and band, so it stays a deliberate choice.

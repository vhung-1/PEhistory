...
- **`asof` = 2026-09-18**; `dates` span **2020-09-21 → 2026-09-18** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`data.json`, `daily_px.json` and `sw_data.json` are all aligned to 2026-09-18** — the price lag below was backfilled the same day.
- Most recent change (**2026-09-20**) advanced the panel one trading day to Fri 18 Sep: 69 of 71 US names and all 20 European names moved. The two unchanged US names (`BILL US`, `WLTH US`) each print identical consecutive values on ~4.8% of days historically, so two of 71 is expected, not staleness. The workbook was pulled Sun 20 Sep, so Sat 19 / Sun 20 are weekend rows the weekday filter drops; no intraday row to exclude. The rolling window dropped 2020-09-18, so the row count is unchanged.
- **The S&P connector outage on 2026-09-20 was handled and then closed out the same day.** While it was down, 18 Sep was left **blank for all 91 names rather than forward-filled** (`realign_dpx_nofill.py`, §10) with `daily_px.json`'s `asof` held at 2026-09-17, so `pxLagNote()` showed the red one-day-behind banner. The connector came back later that day and **`splice_920.py` backfilled 18 Sep in place** — a backfill has a different shape from a normal round because **the axis does not move**: base = the *current* `daily_px.json` (values to 9/17, 9/18 blank), F = median(base/pull) over the Jan–17 Sep overlap as usual. The seam reproduced the pull's own move for 90 of 91 names (`MQ US` 0.125pp, the F=0.25 artifact), `asof` returned to 2026-09-18 and the banner cleared.
- **Zero historical revisions** — the first perfectly clean round. The `SQN SW` HOLD window re-pinned itself. The new-day outlier scan flagged `COIN US` +11.7% and `HOOD US` +9.1%; with the connector down these were cleared on the §10 substitutes, and **once the backfill landed the real price cross-check confirmed every one of those judgements exactly**: COIN P/E +11.65% vs price +11.66% (−0.00pp unexplained), HOOD +9.10% vs +9.12% (−0.02pp), CRCL +7.86% vs +7.86%, FIGR +5.27% vs +5.28%. The two unchanged US names were also vindicated — `BILL US` and `WLTH US` each had a share price that was **exactly flat** that day, so the identical multiples were correct, not stale. Median |move| 1.08%, no level below 3x or above 200x.
- The last spliced price day is **18 Sep** (`splice_920.py`, the backfill described above).

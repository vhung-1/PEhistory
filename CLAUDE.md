...
- **`asof` = 2026-09-16**; `dates` span **2020-09-17 → 2026-09-16** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range. **All 91 names are populated on the last day**, and there is no active `DROP` entry.
- Most recent change (**2026-09-17**) advanced the panel one trading day to Wed 16 Sep: all 71 US and all 20 European names moved, none matching 15 Sep. **This is now the third round running** where the date that carried the §10 stale-US signature as the pull-day row turns out to be a real close once the vendor fills in US pricing a day later — that recurrence is the standing case for excluding the pull-day row rather than publishing it. Thu 17 Sep is the pull-day row now and shows the signature in turn (2 of 71 US changed, all 20 Europeans moved), so it is excluded. The rolling window dropped 2020-09-16, so the row count is unchanged.
- **The quietest round on record: 1 name of sourced revisions.** `WISE LN`, 7 contiguous dates 2026-07-08..07-16 at a median −1.14% (range −1.16..−1.09). Largest single-date change 1.16%. Taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself. The new-day outlier scan was clean: median |move| 1.49%, nothing beyond ±8% (extremes `CHYM US` −7.0% and `MRX US` +5.8%), no level below 3x or above 200x.
- Daily prices spliced for 16 Sep via `splice_917.py`; the seam reproduced the pull's own move for **all 91 names** this round — `MQ US`'s F=0.25 rounding artifact (§10) happened to round consistently. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.

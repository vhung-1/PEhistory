...
- **`asof` = 2026-09-14**; `dates` span **2020-09-15 → 2026-09-14** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range. **All 91 names are populated on the last day**, and there is no active `DROP` entry.
- Most recent change (**2026-09-15**) advanced the panel one trading day to Mon 14 Sep: 70 of 71 US names and all 20 European names moved. The single unchanged US name, `AB US`, is not staleness — **its share price was literally flat (36.09 → 36.09), so an identical multiple is correct**, which the price cross-check confirmed. Tue 15 Sep is the pull-day row and carries the §10 stale-US signature (0 of 71 US names changed while all 20 Europeans moved), so it is excluded. The rolling window dropped 2020-09-14, so the row count is unchanged.
- **The `TOST US` drop has been reversed — the vendor fixed it.** 2026-09-11 now reads **24.29x** (was the corrupt 0.84x), sitting normally between 24.16x on 10 Sep and 25.18x on 14 Sep, so the `DROP` block was removed from `mkdata.py` and the panel is complete again. Otherwise a very quiet round: **4 names of sourced revisions, largest single-date change 0.56%** — `ADYEN NA` 184 scattered dates back to 2025-05-28 at a median −0.10% (it gets a sub-0.4% rounding-level nudge most rounds), `HOOD US` 4 dates at ≈ −0.55%, and a single +0.09% tweak to `V US` on 2026-06-05. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself. The new-day outlier scan was clean: median |move| 1.45%, nothing beyond ±8%, no level below 3x or above 200x.
- Daily prices spliced for 14 Sep via `splice_915.py`; the seam reproduced the pull's own move for **90 of 91 names**, the exception being `MQ US` at 0.060pp — the F=0.25 basis rounding artifact (§10), well inside its ~0.25%/observation budget. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.
...
- **A daily email routine is live** — Claude routine `trig_017Ew43keHj4u8yXBzJ7vLfQ`, 08:00 London, fresh session per firing, running `python pair_email.py --send`. It emails premium/discount charts for 17 curated pairs plus absolute charts for XYZ/ADYEN/CHYM, subject prefixed "Relative P/E Agent". The chart PNGs are rendered from the gate-passed dashboard during the Pages deploy (`mkcharts.mjs` → `_site/charts`), never committed. A one-shot (`trig_01PL9sLBobUMTHi44f8F5uBY`, 2026-10-25) shifts the cron for the BST→GMT change.

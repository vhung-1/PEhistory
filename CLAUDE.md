...
- **`asof` = 2026-09-10**; `dates` span **2020-09-11 → 2026-09-10** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range.
- Most recent change (**2026-09-11**) advanced the panel one trading day to Thu 10 Sep — another clean refresh: 71 US + 18 European names moved on 10 Sep and **none of the 71 US names matched 9 Sep**; Fri 11 Sep is the intraday row (4 US names changed, every US value identical to 10 Sep) and is excluded. The rolling window dropped 2020-09-10, so the row count is unchanged.
- **Sourced revisions: 10 names, largest single-date change 3.28%** — two contiguous multi-date blocks with the textbook restatement signature (tight uniform %, share price moved independently): `TOST US` 6 dates 09-02..09-09 at ≈ −3.27% and `SOFI US` 6 dates at ≈ +2.6%; `FIS US` and `FOUR US` 2 dates each; the remaining six (`CME US`, `CBOE US`, `DB1 GY`, `ENX FP`, `XYZ US`, `LPLA US`) are single-date tweaks to 2026-09-09 as the vendor finalised the prior last row. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself.
- Daily prices spliced for 10 Sep via `splice_912.py`; the seam reproduced the pull's own move for **90 of 91 names**, the exception being `MQ US` at 0.064pp — the F=0.25 basis rounding artifact (§10), well inside its ~0.25%/observation budget. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.

...
- **`asof` = 2026-09-22**; `dates` span **2020-09-23 → 2026-09-22** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`data.json`, `daily_px.json` and `sw_data.json` are all aligned to 2026-09-22.**
- Most recent change (**2026-09-23**) advanced the panel one trading day to Tue 22 Sep: all 71 US and 19 of 20 European names moved, none of the US values matching 21 Sep. Last round this same date was the pull-day row with *nothing* moving; the vendor has since filled it in. Wed 23 Sep is the pull-day row now (0 of 71 US changed, 2 of 20 Europeans moved) and is excluded. The rolling window dropped 2020-09-22, so the row count is unchanged.
- **Sourced revisions: 4 names, 4 cells, largest single-date change 0.23%** — all single-date tweaks to 2026-09-21 as the vendor finalised the prior last row (`FICO US` +0.23%, `CRCL US` −0.14%, `COIN US` −0.10%, `EFX US` +0.06%). No multi-date blocks. Taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself. (`KKR US` stayed at its restored values — the 21 Sep reversal described in §10 held.)
- The new-day outlier scan was clean: median |move| 1.38%, **nothing beyond ±8%**, no level below 3x or above 200x. The largest movers all reconcile to price within 0.02pp — `LPLA US` P/E −7.46% vs price −7.45%, `SCHW US` −6.09% vs −6.11%, `TW US` −4.50% vs −4.51%, `SHOP US` +7.11% vs +7.12%, `FIGR US` +5.16% vs +5.16%.
- The last spliced price day is **22 Sep** (`splice_923.py`); the seam reproduced the pull's own move for 90 of 91 names, `MQ US` differing 0.065pp on the F=0.25 rounding artifact (§10).

...
- **`asof` = 2026-09-04**; `dates` span **2020-09-08 → 2026-09-04** (1,564 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range.
- Most recent change (**2026-09-08**) was a **revisions-only refresh**. `ASOF` did **not** advance past Fri 4 Sep: 5/6 Sep are the weekend, **Mon 7 Sep was US Labor Day** (only the 20 European names moved — see the §10 diagnostic), and the 8 Sep row was today's early intraday pull (6 names). The workbook's rolling 6-year window *did* advance, so the axis lost its oldest row (2020-09-07) and shrank 1,565 → 1,564. Because no trading day was added, **no new S&P pull was needed** — `realign_0908.py` remapped the already-spliced `daily_px.json` onto the new axis by date (0 value changes, single adjustment basis preserved, §7c step 6).
- **Sourced revisions this round:** `AMP US` restated across **41** dates (2026-07-10 → 2026-09-04, ≈ −0.2% each) — a normal vendor forward-EPS restatement, taken as truth. The only other diffs were the 34 **SQN SW** glitch-window cells, which `mkdata.py`'s `HOLD` dict re-pins to the pre-glitch sourced values automatically (§10).
- Backtest headlines on the 74-name tradeable universe: **6-month buy-the-discount reversion ≈ +5.44%; portfolio Sharpe ≈ 1.21** (n = 48). The gate passes 39/39 because engine and reference move together.
- The **software panel is stale by design**: last sourced observation **2026-08-19**, capped by `SW_ASOF` in `mksw.py` (the 2026-08-20 workbook row is a pre-open forward-fill of 8/19 for all 8 names). It only moves when a fresh `bbg_software_mults.xlsx` is supplied (§7f); the tab self-discloses the lag.

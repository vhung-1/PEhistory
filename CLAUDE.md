...
- **`asof` = 2026-09-23**; `dates` span **2020-09-24 → 2026-09-23** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`data.json`, `daily_px.json` and `sw_data.json` are all aligned to 2026-09-23.**
- Most recent change (**2026-09-24**) advanced the panel one trading day to Wed 23 Sep: all 71 US and 19 of 20 European names moved, none of the US values matching 22 Sep. Thu 24 Sep is the pull-day row (1 of 71 US and 1 of 20 European names changed) and is excluded. The single unmoved European name has rotated daily (`BGN IM` 21 Sep, `DB1 GY` 22 Sep, `FBK IM` 23 Sep), so it is an ordinary flat day rather than a stale feed — and FBK's share price was exactly flat on 23 Sep, confirming it. The rolling window dropped 2020-09-23, so the row count is unchanged.
- **Sourced revisions: 3 names, largest single-date change 1.79%.** `WISE LN` 15 contiguous dates 09-02..09-22 at a median +1.65% — a **fresh block**, not a reversal (09-02 had read 19.43x unchanged across the previous eight workbook vintages), with the share price −10.9% across the block and the revised level persisting into the new day. Per the §10 transient-restatement note it is recorded as a new block that may or may not prove durable, not as a settled restatement. Plus `JKHY US` 2 dates −0.11% and `HOOD US` 1 date −0.09%. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself; `KKR US` still at its restored values.
- The new-day outlier scan was clean: median |move| 1.24%, **nothing beyond ±8%**, no level below 3x or above 200x. The largest movers reconcile to price within 0.02pp — `CHYM US` P/E −7.55% vs price −7.55%, `FIGR US` −6.58% vs −6.60%, `KLAR US` −5.36% vs −5.34%. `WISE LN`'s −1.70% P/E vs −0.92% price leaves a 0.77pp gap (a ~0.8% FY+1 EPS uptick); that is ordinary for WISE, whose own daily gap has a median of 0.31pp and exceeds 0.77pp on 16.9% of days (§10).
- The last spliced price day is **23 Sep** (`splice_924.py`); the seam reproduced the pull's own move for 90 of 91 names, `MQ US` differing 0.126pp on the F=0.25 rounding artifact (§10).
...
- Backtest headlines on the 74-name tradeable universe: **6-month buy-the-discount reversion ≈ +5.44%; portfolio Sharpe ≈ 1.21** (n = 48). The gate passes 39/39.

...
- **`asof` = 2026-09-11**; `dates` span **2020-09-14 → 2026-09-11** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range. **90 of 91 names are populated on the last day** — `TOST US` 2026-09-11 is dropped to `null` (see the `DROP` note in §10).
- Most recent change (**2026-09-13**) advanced the panel one trading day to Fri 11 Sep: all 71 US **and** all 20 European names moved, with no US value matching 10 Sep. The workbook was pulled on Sun 13 Sep, so Sat 12 / Sun 13 are weekend forward-fills (0 names change) that the weekday filter drops — there was no intraday row to exclude this round. The rolling window dropped 2020-09-11, so the row count is unchanged.
- **One dropped observation and 6 names of sourced revisions.** The drop: **`TOST US` 2026-09-11 printed 0.84x against 24.16x on 10 Sep — a −96.5% overnight move on a +0.6% share-price move** (31.94 → 32.12), i.e. an FY+1 EPS wrong by ~29x. It is set to `null` via the new `DROP` block in `mkdata.py` rather than published or carried forward (§10). **Remove that block once the vendor corrects the series.** The revisions: `ADYEN NA` 183 scattered dates back to 2025-05-28 at a median +0.10% (sub-0.2% throughout — a rounding-level refinement), `MRX US` 49 contiguous dates 07-06..09-10 at ≈ −5.0% (share price +11.7% across the same block, so price and multiple moved independently — the restatement signature, and the revised level persists rather than snapping back), plus `GPN US` 3 dates and single-date tweaks to `TRU US`, `EXPN LN`, `SHOP US`. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself.
- Daily prices spliced for 11 Sep via `splice_913.py`; the seam reproduced the pull's own move for **90 of 91 names**, the exception being `MQ US` at 0.062pp — the F=0.25 basis rounding artifact (§10), well inside its ~0.25%/observation budget. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.

...
- **`asof` = 2026-09-17**; `dates` span **2020-09-18 → 2026-09-17** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range. **All 91 names are populated on the last day**, and there is no active `DROP` entry.
- Most recent change (**2026-09-18**) advanced the panel one trading day to Thu 17 Sep: all 71 US and all 20 European names moved, none matching 16 Sep. **Fourth round running** that the prior pull-day row turns out to be a real close once the vendor fills in US pricing a day later — the standing case for excluding it rather than publishing it. Fri 18 Sep is the pull-day row now and shows the §10 stale-US signature in turn (1 of 71 US changed, all 20 Europeans moved), so it is excluded. The rolling window dropped 2020-09-17, so the row count is unchanged.
- **Sourced revisions: 3 names, largest single-date change 4.38%**, all with the restatement signature (price moving independently of the multiple): `BAM US` 49 contiguous dates 2024-02-13..04-19 at a median −1.11% (price −2.2% across the block), `XYZ US` 47 scattered dates 2024-03..2025-11 at a median +0.74%, and `KKR US` 6 contiguous dates 09-09..09-16 at ≈ −2.74% (price −7.0% across the block). All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself. The new-day outlier scan was clean: median |move| 1.01%, nothing beyond ±8%, no level below 3x or above 200x.
- Daily prices spliced for 17 Sep via `splice_918.py`; the seam reproduced the pull's own move for **90 of 91 names**, the exception being `MQ US` at 0.126pp — the F=0.25 basis rounding artifact (§10); the day's move was tiny (−0.25%), so a single 2-dp rounding step is proportionally larger, still inside the ~0.25%/observation budget. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.
- **`sw_data.json` was realigned with `realign_sw.py`, not rebuilt** — this container had no `bbg_software_mults.xlsx` (gitignored), so `mksw.py` could not run. See the §10 note; the realign is a pure date re-index of already-sourced values, verified by `sw['asof'] == data['asof']` and the `SW_ASOF` row landing on 2026-08-19.

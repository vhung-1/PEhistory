...
- **`asof` = 2026-09-09**; `dates` span **2020-09-10 → 2026-09-09** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range.
- Most recent change (**2026-09-10**) advanced the panel one trading day to Wed 9 Sep — a clean, unremarkable refresh: all 71 US names moved on 9 Sep and none matched 8 Sep; Thu 10 Sep is the intraday row (2 US names, every US value identical to 9 Sep) and is excluded. The rolling window dropped 2020-09-09, so the row count is unchanged.
- **Sourced revisions: 14 names, largest single-date change 0.95%** — the smallest round in a while. Three multi-date blocks (`HLNE US` 36 dates ≈ +0.3%, `AMP US` 28, `FIGR US` 16 ≈ −0.5%); the other 11 are single-date tweaks to 2026-09-08 as the vendor finalised the prior last row. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself.
- Daily prices spliced for 9 Sep via `splice_911.py`; the seam reproduced the pull's own move for **all 91 names** (`MQ US`'s F=0.25 rounding artifact, §10, happened to round consistently this round). The usual 7 European names flag the recurring 2026-07-03 US-holiday forward-fill artifact only.
- **Mon 7 Sep (US Labor Day) sits in the panel as an interior row**, US forward-filled and Europe live — the six-year precedent (§10). The `ASOF` cap protects the *published* date; an interior holiday is a known one-day artifact.

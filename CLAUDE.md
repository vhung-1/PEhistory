...
- **`asof` = 2026-09-08**; `dates` span **2020-09-09 → 2026-09-08** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range.
- Most recent change (**2026-09-09**) advanced the panel by **one trading day**, to Tue 8 Sep. It took three workbook uploads that day to get there, which is worth knowing about:
  - the first upload was `Coverage_dashboards.xlsx`, the investor's master working workbook — 18 sheets, and its `Mult history` sheet carries forward P/E for only **8 names**. Cross-checked 32/32 cells against the panel (identical), so it is the same Bloomberg feed and the same FY+1 definition (`pe_ratio(fpt=a,fpo=2)`), but it cannot drive a 91-name refresh.
  - the second upload ran to 8 Sep but **did not contain Tuesday's US closes**: 64 of 71 US names were byte-identical to 4 Sep to 17 decimal places, while all 20 Europeans had moved. `ASOF` was held at 4 Sep — see the §10 diagnostic. That round still shipped three sourced restatements (WISE LN, SQN SW, DWS GY).
  - the third upload had them: all 71 US names moved on 8 Sep and none matched 4 Sep. `ASOF` = **2026-09-08**; Wed 9 Sep is the intraday row (8 US names) and is excluded.
- **Mon 7 Sep (US Labor Day) is kept as an interior row**, US forward-filled and Europe live. That is the six-year precedent — every past US holiday is present the same way (Labor Day 2025: 65/71 US unchanged, 2/20 EU). The `ASOF` cap protects the *published* date, where stale US prices would corrupt every pair's current reading; an interior holiday is a known one-day artifact.
- **Sourced revisions that round: 18 names**, all contiguous and **none exceeding 8% on any date** — routine vendor forward-EPS restatements, taken as truth (§7a). Largest: `MRX US` 25 dates ≈ −5.2%; then `CBOE US` 59 dates ≈ −0.3%, `ENX FP` 57 ≈ −0.6%, `CHYM US` 40, `WISE LN` 35, `HLNE US` 34. The `SQN SW` HOLD window (§10) re-pinned itself automatically.
- Daily prices spliced for 7–8 Sep via `splice_910.py`; the seam reproduces the pull's own moves for 90 of 91 names (the exception is the `MQ US` rounding artifact in §10). The usual 7 European names flag the recurring 2026-07-03 US-holiday forward-fill artifact only.

...
- **`asof` = 2026-09-18**; `dates` span **2020-09-21 → 2026-09-18** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`daily_px.json` is deliberately one day behind at `asof` 2026-09-17** — see the next bullet. `sw_data.json` is aligned to 2026-09-18.
...
- **⚠️ The S&P Global connector was disconnected and needed re-authorization**, which a non-interactive session cannot perform, so **Fri 18 Sep closes could not be fetched**. `daily_px.json` was realigned with `realign_dpx_nofill.py` — 18 Sep is left **blank for all 91 names rather than forward-filled** (§10), and its `asof` stays at 2026-09-17, which makes `pxLagNote()` render the red one-day-behind banner on the Pairs tab. The P/E panel is unaffected (it comes from the workbook), and `DPX` feeds nothing but click-to-measure. **Backfill 18 Sep with the normal splice once the connector is authorized again.**
- **Zero historical revisions** — the first perfectly clean round. The `SQN SW` HOLD window re-pinned itself. The new-day outlier scan flagged two names beyond ±8%, `COIN US` +11.7% and `HOOD US` +9.1%; **the price cross-check could not be run** (connector down), so they were cleared on the substitutes in §10 — both exceed ±8% on ~10% of all days with a p99 near 25%, and the whole crypto complex moved together in a coherent gradient (COIN +11.7%, HOOD +9.1%, CRCL +7.9%, FIGR +5.3%). Median |move| 1.08%, no level below 3x or above 200x.
- The last spliced price day is **17 Sep** (`splice_918.py`); 18 Sep is pending the connector, as above.

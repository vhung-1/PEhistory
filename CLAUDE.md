...
- **`asof` = 2026-09-21**; `dates` span **2020-09-22 → 2026-09-21** (1,565 weekday rows); **91 tickers**; 0 weekend rows; all 91 populated on the last day; no active `DROP` entry. **`data.json`, `daily_px.json` and `sw_data.json` are all aligned to 2026-09-21.**
- Most recent change (**2026-09-22**) advanced the panel one trading day to Mon 21 Sep: all 71 US and 19 of 20 European names moved, none of the US values matching 18 Sep. **Tue 22 Sep is the starkest form of the §10 pull-day signature yet — nothing moved at all** (0 of 71 US and 0 of 20 European names), i.e. the pull landed before any market data refreshed; excluded. Sat 19 / Sun 20 are weekend rows the weekday filter drops. The rolling window dropped 2020-09-21, so the row count is unchanged.
- **Sourced revisions: 6 names, largest single-date change 6.47%.** `SHOP US` 48 contiguous dates 2024-03-07..05-13 at a median −5.97% (share price −22.0% across the block, so price and multiple moved independently — the restatement signature); `MA US` 37 scattered dates at a median −0.03% (rounding-level); `FOUR US` 9 dates ≈ −1.94%; **`KKR US` 8 dates ≈ +2.82%, which exactly reverses the cut the 17 Sep workbook made** — see the transient-restatement note in §10; plus single-date tweaks to `SCHW US` and `IBKR US`. All taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself.
- The new-day outlier scan was clean: median |move| 1.34%, **nothing beyond ±8%**, no level below 3x or above 200x. The three largest movers all reconcile to price — `SHOP US` P/E +7.34% vs price +7.33%, `FTK GY` +7.30% vs +7.35%, `SAVE SS` +5.74% vs +5.72%.
- The last spliced price day is **21 Sep** (`splice_922.py`); the seam reproduced the pull's own move for 90 of 91 names, `MQ US` differing 0.129pp on the F=0.25 rounding artifact (§10).

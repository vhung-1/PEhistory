...
- **`asof` = 2026-09-15**; `dates` span **2020-09-16 → 2026-09-15** (1,565 weekday rows); **91 tickers**; 0 weekend rows. `daily_px.json` and `sw_data.json` both span the same range. **All 91 names are populated on the last day**, and there is no active `DROP` entry.
- Most recent change (**2026-09-16**) advanced the panel one trading day to Tue 15 Sep: all 71 US and all 20 European names moved, none matching 14 Sep. **This is the same date that carried the stale-US signature last round** (0 of 71 US changed) when it was the pull-day row — the vendor has since filled in real US pricing for it, which is precisely why holding it back was right. Wed 16 Sep is now the pull-day row and shows that signature in turn (1 of 71 US changed, all 20 Europeans moved), so it is excluded. The rolling window dropped 2020-09-15, so the row count is unchanged.
- **Sourced revisions: 2 names, largest single-date change 1.53%.** `AMP US` 39 scattered dates 2024-01..2025-05 at a median +0.09%, and `CBOE US` 32 **contiguous** dates 07-31..09-14 at ≈ −1.1% — the restatement signature (share price −8.3% across the same block, so price and multiple moved independently, and the revised level persists with no snap-back). Both taken as truth (§7a). The `SQN SW` HOLD window re-pinned itself. **The new-day outlier scan flagged two names beyond ±8% and both cleared on the price cross-check:** `CRCL US` −11.4% P/E vs −11.41% price and `COIN US` −10.1% vs −10.10% — a genuine crypto reversal after Monday's rally, not a data artifact.
- Daily prices spliced for 15 Sep via `splice_916.py`; the seam reproduced the pull's own move for **90 of 91 names**, the exception being `MQ US` at 0.057pp — the F=0.25 basis rounding artifact (§10), well inside its ~0.25%/observation budget. The usual 7 European names (AMUN FP, BGN IM, DB1 GY, ENX FP, EQT SS, ICG LN, SQN SW) flag the recurring 2026-07-03 US-holiday forward-fill artifact only.
...
- **A daily email routine is live** — Claude routine `trig_017Ew43keHj4u8yXBzJ7vLfQ`, 08:00 London, fresh session per firing, running `python pair_email.py --send`. It emails premium/discount charts for 17 curated pairs plus absolute charts for XYZ/ADYEN/CHYM/**MRX** (Marex added 2026-09-16), subject prefixed "Relative P/E Agent". The chart PNGs are rendered from the gate-passed dashboard during the Pages deploy (`mkcharts.mjs` → `_site/charts`), never committed. **The pair/single lists live in THREE places that must move together** — `SINGLES`/`PAIRS` in `pair_email.py`, the same two arrays in `mkcharts.mjs`, and the render-count assertion in `.github/workflows/pages.yml` (currently `-eq 21` = 17 pairs + 4 singles). Miss the workflow count and the deploy fails; miss `mkcharts.mjs` and the email links a 404. A one-shot (`trig_01PL9sLBobUMTHi44f8F5uBY`, 2026-10-25) shifts the cron for the BST→GMT change.
